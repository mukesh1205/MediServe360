...
public class VitalsController {
	...
	@PostMapping("/add")
	public ResponseEntity<VitalsResponseDTO> addVitals(@RequestBody VitalsRequestDTO request) {
		return ResponseEntity.ok(vitalsService.addVitals(request));
	}

	@GetMapping("/patient/{patientId}")
	public ResponseEntity<List<VitalsResponseDTO>> getByPatient(@PathVariable int patientId) {
		return ResponseEntity.ok(vitalsService.getVitalsByPatient(patientId));
	}

	@GetMapping("/{vitalId}")
	public ResponseEntity<VitalsResponseDTO> getById(@PathVariable Long vitalId) {
